...
class DataCollector(object):
  ...
  def __init__(self, serial_obj, save_dir):
    ...
    self.img_file=self.save_base+'/imgs_{0}'.format(nowtime.strftime(time_format))
    self.IMUdata_file=self.save_base+'/IMU_{0}'.format(nowtime.strftime(time_format))
    self.RCcommands_file=self.save_base+'/commands_{0}'.format(nowtime.strftime(time_format))
 
  def set_save_dir(self):
    nowtime=datetime.datetime.now()
    self.save_dir=self.save_base+"/"+format(nowtime.strftime(time_format))
...
def imageprocessor(event, serial_obj):
  ...
  with g_graph.as_default():
    ...
    while not event.is_set():
      ...
      (str_c, thr_c)=filter_out(pred, g_steerstats) 

      end=time.time()
      print(end-start)
      dataline='{0}, {1}, {2}, {3}\n'.format(commandEnum.RUN_AUTONOMOUSLY, int(str_c), int(thr_c), 0)
      print(dataline)
      try:
...
#code is an int in range 0-7, consisting of binary on-off values for the leds. boot_RPi is MSB
def displayBinLEDCode(code): 
  GPIO.output(LED_names["shutdown_RPi"], (code>>2)&1)
  GPIO.output(LED_names["autonomous"], (code>>1)&1)
